...
describe("payrollTabGroups", () => {
  it("filters groups to visible tabs only", () => {
    const groups = getVisiblePayrollTabGroups(["employees", "attendance", "payroll"]);
    expect(groups).toHaveLength(3);
    expect(groups[0]?.tabs).toEqual(["employees"]);
    expect(groups[1]?.tabs).toEqual(["attendance"]);
    expect(groups[2]?.tabs).toEqual(["payroll"]);
  });

  it("finds group containing the active tab", () => {
    const groups = getVisiblePayrollTabGroups(["employees", "attendance", "overtime", "payroll"]);
  ...
  it("falls back to first group when tab is not in any group", () => {
    const groups = getVisiblePayrollTabGroups(["employees", "attendance"]);
